    ...
    def evaluate(self, loader):
        ...
        with torch.no_grad():
            ...
            for step, (index, past, future, presents, angle_presents, videos, vehicles, number_vec, scene, scene_one_hot) \
                ...
                # fde_distances = distances[:, :, -1]
                # index_min = torch.argmin(fde_distances, dim=1)
                distance_pred = distances[torch.arange(0, len(index_min)), index_min]

                for i in range(pred.shape[0]):
                    key = str(index[i].item()) + '_' + videos[i] + '_' + number_vec[i]
                    past_dict[key] = past[i].cpu().numpy()
                    future_dict[key] = future[i].cpu().numpy()
                    scene_dict[key] = scene[i].cpu().numpy()
                    preds_dict[key] = pred[i].cpu().numpy()
                    errors_dict[key] = [distance_pred[i, 9], distance_pred[i, 19], distance_pred[i, 29], distance_pred[i, 39]]
            ...
            # with open(self.folder_test + 'errors_dict.pkl', 'wb') as handle:
            #     pickle.dump(errors_dict, handle)
            with open(self.folder_test + 'pasts.pkl', 'wb') as handle:
                pickle.dump(past_dict, handle)
            with open(self.folder_test + 'futures.pkl', 'wb') as handle:
                pickle.dump(future_dict, handle)
            with open(self.folder_test + 'scenes.pkl', 'wb') as handle:
