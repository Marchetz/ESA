...
class model_tran(nn.Module):
    ...
    def forward(self, past, scene=None, future=None):
        ...
        encoder_output = self.transformer_model.encoder(src)
        tgt_query = query.repeat(self.num_prediction,1,1).cuda()
        
        random_int_tensor = torch.randint(low=0, high=self.num_prediction, size=(dim_batch, self.num_prediction), dtype=torch.long).cuda()
             
        embeddibing = self.embedding(random_int_tensor).permute(1,0,2)
        
        embeddibing = self.dropout(embeddibing)
        
                                 
        tgt = torch.cat((tgt_query,embeddibing), -1).cuda()
        
        memory = encoder_output

        decoder_output = self.transformer_model.decoder(tgt, memory)

        #transformer = self.transformer_model(src, tgt)                 
        out_weight = []
        
        info_future = torch.reshape(decoder_output, (1,self.num_prediction*dim_batch,self.transformer_model.d_model))
