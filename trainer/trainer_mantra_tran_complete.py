...
from models.model_tran import model_tran
from models.model_tran_complete import model_tran_complete
import io
...
class Trainer:
    def __init__(self, config):
        ...
        self.model_IRM = torch.load(config.model_IRM)

        self.mem_n2n = model_tran_complete(self.settings, self.model_ae, self.model_controller, self.model_IRM)
    ...
    def save_plot_weight(self, title):
        ...
        plt.grid()
        plt.savefig('function_memory_%s.png' % title)
    ...
    def fit(self):
        ...
        step_results = [1, 10, 20, 30, 40, 50, 60, 80, 90, 100, 120, 150, 170, 200, 250, 300, 350, 400, 450, 490, 550, 600,650,700,750,800,850,900,950,999, 1000]
        # Main training loop
    ...
    def draw_track(self, past, future, scene, pred=None, angle=0, video_id='', vec_id='', index_tracklet=0,
        ...
        if pred is not None:
            for i_p in reversed(range(pred.shape[0])):
                pred_i = cv2.transform(pred[i_p].cpu().type(torch.FloatTensor).numpy().reshape(-1, 1, 2), matRot_track).squeeze()
    ...
    def _train_single_epoch(self):
        ...
        config = self.config
        self.mem_n2n.train()
        for step, (index, past, future, _, _, _, _, _, _, scene_one_hot, scene_intention) in enumerate(tqdm.tqdm(self.train_loader)):
            self.opt.zero_grad()
            with torch.cuda.amp.autocast(enabled=True):
                self.iterations += 1
                if config.cuda:
                    ...
                    future = future.cuda()
                    scene_one_hot = scene_one_hot.cuda()
                    scene_intention = scene_intention.cuda()

                output, _, _, _ = self.mem_n2n(past, scene_intention, scene_one_hot, future)
            ...
            torch.nn.utils.clip_grad_norm_(self.mem_n2n.parameters(), 1.0)
    ...
    def _memory_writing(self, saved_memory):
        ...
        if saved_memory:
            ...
            with torch.cuda.amp.autocast(enabled=True):
                ...
                config = self.config
                with torch.no_grad():
                    for step, (index, past, future, _, _, _, _, _, _, scene_one_hot, scene_intention) in enumerate(self.train_loader):
                        self.iterations += 1
                        if config.cuda:
                            past = past.cuda()
                            future = future.cuda()
                            scene_one_hot = scene_one_hot.cuda()
                            scene_intention = scene_intention.cuda()
                        self.mem_n2n(past, scene_intention, scene_one_hot, future)
